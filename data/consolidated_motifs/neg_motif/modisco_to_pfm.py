...
if __name__=='__main__':
    ...
    if 'metacluster_1' in f['metacluster_idx_to_submetacluster_results']:
        num_patterns = len(f['metacluster_idx_to_submetacluster_results']['metacluster_1']['seqlets_to_patterns_result']['patterns'])-1
    
        for i in range(num_patterns):
            trimmed_ppm = trim_ppm(f['metacluster_idx_to_submetacluster_results']['metacluster_1']['seqlets_to_patterns_result']['patterns']['pattern_{}'.format(i)]['sequence']['fwd'], 
                    t=args.threshold, min_length=args.min_length, flank=args.flank_add)

            if trimmed_ppm is not None:
                num_seqlets = len(f['metacluster_idx_to_submetacluster_results']['metacluster_1']['seqlets_to_patterns_result']['patterns']['pattern_{}'.format(i)]['seqlets_and_alnmts']['seqlets'])
                ...
                if num_seqlets >= args.min_seqlets:
                    ...
                    if args.normalize:
                        ...
                        np.savetxt("{}.pattern_{}.pfm".format(args.output_prefix, i),
