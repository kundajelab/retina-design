...
def fetch_tomtom_matches(ppm, background=[0.25, 0.25, 0.25, 0.25], tomtom_exec_path='tomtom', motifs_db='HOCOMOCOv11_core_HUMAN_mono_meme_format.meme' , 
        n=5, trim_threshold=0.45, trim_min_length=3, trim_flank=0):
    ...
    # trim pfm
    trimmed = modisco_to_pfm.trim_ppm(ppm, t=trim_threshold, min_length=trim_min_length, flank=trim_flank)
...
def fetch_args():
    ...
    parser.add_argument("-tf", "--trim_flank", type=int, default=0, help="Flank added to trimmed motif")
...
if __name__=="__main__":
    ...
    num_patterns = len(f['neg_patterns']) if 'neg_patterns' in f else 0

    for i in range(num_patterns):
        ppm = f['neg_patterns'][f'pattern_{i}']['sequence'][:]
        num_seqlets = f['neg_patterns'][f'pattern_{i}']['seqlets']['n_seqlets'][:][0]
        ppms.append(ppm)
    ...
    for i,x in enumerate(ppms):
        res.append(fetch_tomtom_matches(x, tomtom_exec_path=args.tomtom_exec, motifs_db=args.meme_motif_db, 
            n=args.top_n_matches, trim_threshold=args.trim_threshold, trim_min_length=args.trim_min_length, trim_flank=args.trim_flank)) 
