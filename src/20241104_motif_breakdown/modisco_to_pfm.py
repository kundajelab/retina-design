...
if __name__=='__main__':
    ...
    # direction can be 'pos_patterns' and/or 'neg_patterns'
    for direction in f.keys():
        num_patterns = len(f[direction])
    
        for i in range(num_patterns):
            trimmed_ppm = trim_ppm(f[direction][f'pattern_{i}']['sequence'][:],
                    t=args.threshold, min_length=args.min_length, flank=args.flank_add)
    
            if trimmed_ppm is not None:
                num_seqlets = f[direction][f'pattern_{i}']['seqlets']['n_seqlets'][:]
    
                if num_seqlets >= args.min_seqlets:
                    pfm = (trimmed_ppm*num_seqlets).astype(np.int32)
                
                    if args.normalize:
                        pfm = (pfm+1)/np.sum(pfm, axis=1, keepdims=True)
                        np.savetxt(os.path.join(args.output_dir, "{}_pattern_{}.pfm".format(direction.split("_")[0], i)),
